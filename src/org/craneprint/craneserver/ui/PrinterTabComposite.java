...
public class PrinterTabComposite extends CustomComponent {
	...
	public PrinterTabComposite(int pid, Craneprint_craneserverUI u, boolean showDisabled) {
		printerId = pid;
		ui = u;
		enabled = ui.getDBManager().isPrinterActive(printerId);
		showDis = showDisabled;
		if(enabled || showDis){
			buildMainLayout();
	...
	public PrinterTabComposite(int pid, Craneprint_craneserverUI u) {
		...
		ui = u;
		enabled = ui.getDBManager().isPrinterActive(printerId);
		Accordion a = (Accordion)this.getParent();
		if(enabled || showDis){
			buildMainLayout();
			setCompositionRoot(mainLayout);
		}
	...
	protected void fillComposite() {
		if (ui.getDBManager().isPrinterActive(printerId)) {
			...
			try {
				...
				queueVLabel.setValue(new Integer(ui.getDBManager()
						.getQueueSize(printerId)).toString());
				notesArea.setValue("");
				...
				new Notification("Could Not Connect", "Printer is Offline",
				...
				if (hs != null) {
					stateVLabel.setValue(PrinterStatus.getStringForInt(hs
							.getStatus()));
					queueVLabel.setValue(new Integer(ui.getDBManager()
							.getQueueSize(printerId)).toString());
			...
			try {
				...
				stateVLabel.setValue("DISABLED BY ADMIN");
				queueVLabel.setValue(new Integer(ui.getDBManager()
						.getQueueSize(printerId)).toString());
				notesArea.setValue("");
				// Reset the tools stuff
